	...
	private void produce() throws InterruptedException {
		...
		while (bufferCondiviso.size() >= SIZE) {
			synchronized (bufferCondiviso) {
				System.out.println("Il buffer è pieno, "+Thread.currentThread().getName()+" resta in attesa... la dimensione del buffer adesso è: " + bufferCondiviso.size());
		...
		synchronized (bufferCondiviso) {
			...
			System.out.println(Thread.currentThread().getName()+" ha aggiunto al buffer l'elemento: " + i + " la dimensione del buffer adesso è: " + bufferCondiviso.size());
		}
	}
}